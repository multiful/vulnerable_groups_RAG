...
> **참조 규칙**: 프롬프트 역할, 입력 슬롯, 출력 형식, 검증 규칙, 근거 사용 원칙을 변경할 때 먼저 이 문서를 수정한다.

---

## 1. 문서 목적

이 문서는 LLM 또는 프롬프트 기반 구성 요소가 **무엇을 입력받고 어떤 원칙으로 응답을 생성해야 하는지**를 정의한다.  
본 시스템에서 프롬프트는 추천 결과를 직접 계산하는 핵심 엔진이 아니라, **설명 정리, 근거 결합, 로드맵 문장화, 응답 형식 제어**에 사용된다.
...
`related_domains`, `related_jobs`, `primary_domain`은 내부 canonical 값 밖의 새 라벨을 생성하지 않는다.

### 2.5 결과 형식 고정
프롬프트 출력은 화면과 API가 기대하는 구조를 따라야 하며, 불필요한 산문형 장문 응답을 만들지 않는다.

### 2.6 프롬프트는 보조 계층
프롬프트는 사용자 응답의 표현 계층이며, 추천의 사실 기반 또는 정책 판단 계층이 아니다.
...
3. evidence가 없으면 “설명 근거 없음” 또는 evidence 생략을 허용한다.
...
8. reserved 필드를 임의 생성하지 않는다.
9. 근거 기반 설명과 구조 기반 설명을 혼합하더라도 출처 성격을 혼동하지 않는다.

---

## 5. Prompt Runtime Contract

프롬프트 호출 전후에는 아래 계약을 따른다.

### 5.1 입력 전제
- 입력 데이터는 가능한 한 canonicalized 상태여야 한다.
- 프롬프트는 정규화되지 않은 자유 텍스트를 직접 정책 판단에 사용하지 않는다.
- evidence snippet은 retrieval 계층이 제공한 결과만 사용한다.

### 5.2 출력 전제
- 출력은 JSON 또는 구조화 가능한 필드 묶음이어야 한다.
- 화면/API에서 바로 사용할 수 있어야 한다.
- 필드 누락 시 후단 validator가 차단할 수 있어야 한다.

### 5.3 검증 주체
프롬프트 출력 검증은 아래 계층에서 수행한다.

- 1차: prompt output parser
- 2차: schema validator
- 3차: business rule checker

즉, 프롬프트 출력은 모델이 직접 확정하지 않고 **후단 검증 계층**을 통과해야 최종 응답에 포함된다.

---

## 6. 활성 프롬프트 레지스트리

| Prompt ID | 프롬프트명 | 목적 | 상태 |
|---|---|---|---|
| P-01 | Recommendation Explanation Prompt | 추천 결과 설명 생성 | 활성 |
| P-02 | Roadmap Explanation Prompt | 로드맵 단계 설명 생성 | 활성 |
| P-03 | Evidence Summary Prompt | 문서 근거 요약 | 활성 |
| P-04 | Final Response Composer Prompt | 최종 사용자 응답 조립 | 활성 |
| R-01 | Taxonomy Normalization Prompt | 자유 텍스트 정규화 | reserved |
| R-02 | Schedule Summary Prompt | 일정/링크 요약 | reserved |
| R-03 | Conversational Counseling Prompt | 장기 상담형 대화 | reserved |
...
### 출력 원칙
- `summary`는 1~2문장으로 제한
- `related_jobs`, `related_domains`, `primary_domain`의 연결 이유를 설명
- evidence가 있으면 근거 기반 설명을 포함
- evidence가 없으면 구조적 추천 이유만 설명
- 사용자가 바로 이해할 수 있는 수준으로 짧게 쓴다
...
### 검증 규칙
- 세 필드 모두 string이어야 한다.
- 빈 문자열 출력 금지
- taxonomy 밖 라벨 생성 금지

### fallback
- 파싱 실패 시 템플릿 기반 summary만 반환 가능
- evidence가 없으면 evidence 기반 문장은 제거한다
...
### 검증 규칙
- `roadmap_overview`는 string이어야 한다.
- `stage_descriptions`는 배열이어야 한다.
- 단계 수는 입력 `roadmap_stages`와 의미상 대응되어야 한다.

### fallback
- 파싱 실패 시 단계명만 나열하는 간단한 설명 구조로 대체할 수 있다.
...
### 검증 규칙
- evidence 입력이 비어 있으면 빈 결과 허용
- `evidence_points`는 배열이어야 한다.
- evidence가 없는데 임의 사실 생성 금지

### fallback
- evidence가 없으면 `evidence_summary = null`, `evidence_points = []` 허용
...
- recommendation explanation과 evidence summary를 중복 서술하지 않음
...
### 검증 규칙
- 세 필드 모두 string이어야 한다.
- `next_action`은 행동 문장이어야 한다.
- reserved 기능을 완료된 기능처럼 표현하지 않는다.

### fallback
- 조립 실패 시 recommendation explanation의 summary를 headline/body에 축약 반영할 수 있다.
...
- 미확정 정책 값
- 원본 장문 문서 전체
- source profiling 전 원문 전체 데이터

---

## 10. Output Contract 원칙
...
6. parser → validator → business rule checker 순서를 통과해야 한다.
...
2. schema validator 실패 시 해당 프롬프트 출력은 폐기하거나 축약 fallback을 사용한다.
3. evidence가 없으면 evidence summary를 생략한다.
4. reserved 정보가 요청되면 “현재 미연동” 상태를 유지한다.
5. taxonomy 밖 라벨이 생성되면 응답을 폐기하거나 후처리 검증에서 차단한다.

---

## 13. 프롬프트 버전 관리

각 프롬프트는 아래 메타 정보를 가진다.
...
- few-shot 변경만으로도 version을 갱신할 수 있다.
- API 응답이 프롬프트 구조에 의존하면 계약도 함께 점검한다.
- 프롬프트 문구를 수정하면 이 문서의 `최종 수정일`도 함께 갱신한다.
