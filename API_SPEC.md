...
추천 결과와 설명 근거는 같은 응답에서 결합될 수 있지만, 내부적으로는 다른 계층에서 만들어진다.  
API 계약은 이 두 결과를 한 응답 안에서 명시적으로 구분할 수 있어야 한다.

### 2.4 taxonomy 밖 값 금지
API 응답의 `related_domains`, `related_jobs`는 사전 정의된 taxonomy 값만 반환해야 한다.

### 2.5 일정 기능 reserved
일정/접수/링크 endpoint는 현재 reserved 상태이며, 추후 활성화한다.
...
| 기능 ID | Method | Endpoint | 상태 |
|---|---|---|---|
| F-01/F-02/F-03 | POST | `/recommendations` | 활성 |
| F-04 | POST | `/recommendations/evidence` | 활성 |
| F-05 | POST | `/roadmaps` | 활성 |
| F-06 | POST | `/admin/canonicalize` | 활성 |
| F-07 | POST | `/admin/candidates/rebuild` | 활성 |
| F-07 | GET | `/admin/validation` | 활성 |
| - | GET | `/health` | 활성 |
| F-08 | GET | `/schedules/exams/{cert_id}` | reserved |
| F-09 | GET | `/schedules/applications/{cert_id}` | reserved |
| F-09 | GET | `/links/support/{cert_id}` | reserved |
...
### Request 필드
- `risk_stage_id`: 필수
- `interested_jobs`: 선택, taxonomy 기준 직무 배열
- `interested_domains`: 선택, taxonomy 기준 도메인 배열
- `query_text`: 선택, 자유 텍스트 입력
